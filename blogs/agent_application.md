...
In this setup, an *orchestrator*--a custom tool-chain--is built to implement the agent's logic, while integrating with all enterprise assets—a very naive approach, wherein only a few aspects of LLM are employed. The bulk of the logic, most of the business flow, and all integrations are retained in the orchestrator. Many RAG use cases and classical-AI use cases typically employ this use case. 
...
Our rudimentary implementations involved building agents for CRM, demonstrating their capabilities. To demonstrate the possibilities of implementing increasingly complex agents, we chose to implement agents that can *manage customer relations*. The toolset we chose was Python, Google ADK, Ollama, and Llama/Gemma/Gemini-Flash. The CRM solution was HubSpot. 

### HubSpot Taskbot - Level-1 Agents

As part of building a Level-1 agent (which we term `taskbots`), we took on the task of building a CRM client agent. TaskBots are aimed at automating simple, rule-based tasks. They employ LLMs or SLMs to leverage a targeted set of features such as language understanding, conversational ability, and content generation, to achieve repetitive workflows with minimal decision-making. 

The idea was that an agent can look at emails from clients (or potential clients), automatically extract the relevant information, and update the CRM.  Our efforts are in [this GitHub repo](https://github.com/Mildogrc/agent-evolution/). An outline of our implementation efforts is this:
...
### A2A Complexity
We realize that multi-agent solutions (with A2A) are complicated, as many of the challenges of distributed computing (consensus problems, Byzantine faults, stabilization challenges) are amplified because of LLM challenges such as hallucinations and lack of context. 
